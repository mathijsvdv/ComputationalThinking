...
# ╔═╡ dcb6760a-f37f-11ea-3ae1-65443ef5a81a
@bind run_tests CheckBox(default=false)

# ╔═╡ dcb6760a-f37f-11ea-3ae1-65443ef5a81b
...
`remove_in_each_row` has 1029 allocs (342 * 3 + 3), whereas `remove_in_each_row_no_vcat` only has 687 allocs (342 * 2 + 3).

The @benchmark output shows in the improved method that we are making 342 less
calls to alloc.
"""
...
# ╔═╡ 1db3bf88-6882-11eb-25af-6941caa47dd0
img2 = copy(img)

# ╔═╡ 3335e07c-f328-11ea-0e6c-8d38c8c0ad5b
performance_experiment_views = @benchmark begin
	remove_in_each_row_views(img2, 1:size(img2, 1))
end

# ╔═╡ 7265571a-686b-11eb-1411-3fea7ade3e38
remove_in_each_row_views(img2, 1:size(img2, 1))
...
function greedy_seam(energies, r, i)
	h,w = size(energies)
	if i <= 1
		return energies[r+1, i+1] <= energies[r+1, i] ? i+1 : i
	elseif i >= w
		return energies[r+1, i-1] <= energies[r+1, i] ? i-1 : i
	else
		min_idx = energies[r+1, i-1] <= energies[r+1, i] ? i-1 : i
		return energies[r+1, min_idx] <= energies[r+1, i+1] ? min_idx : i+1
	end
end
...
function greedy_seam(energies, starting_pixel::Int)
	...
	reduce((a,b) -> 
		[a..., greedy_seam(energies, b, last(a))], 
		1:h-1; 
		init=[starting_pixel])
end
...
function least_energy(energies, i, j)
	...
	if i >= h-1
		if j == 1
			if energies[i+1, j] < energies[i+1, j+1] 
				return (energies[i,j] + energies[i+1, j], [j])
			else 
				return (energies[i,j] + energies[i+1, j+1], [j+1])
			end
		elseif j == w
			if energies[i+1, j] < energies[i+1, j-1] 
				return (energies[i,j] + energies[i+1, j], [j])
			...
			if energies[i+1,j-1] < energies[i+1,j+1] < energies[i+1,j]
				return (energies[i,j] + energies[i+1,j-1], [j-1])
			elseif energies[i+1,j+1] < energies[i+1,j]
				return (energies[i,j] + energies[i+1,j+1], [j+1])
			else
				return (energies[i,j] + energies[i+1,j], [j])
			end
		...
		if j == 1
			down_energy = least_energy(energies, i+1, j)
			right_energy = least_energy(energies, i+1, j+1)
			if (down_energy[1] + energies[i,j]) < (right_energy[1] + energies[i,j])
				return (
					down_energy[1] + energies[i,j], 
					# push!(down_energy[2], j)
					pushfirst!(down_energy[2], j)
				)
			else
				return (
					right_energy[1] + energies[i,j], 
					# push!(right_energy[2], j)
					pushfirst!(right_energy[2], j+1)
				)
			end
		elseif j == w
			left_energy = least_energy(energies, i+1, j-1)
			down_energy = least_energy(energies, i+1, j)
			if (down_energy[1] + energies[i,j]) < (left_energy[1] + energies[i,j])
				return (
					down_energy[1] + energies[i,j], 
					# push!(down_energy[2], j)
					pushfirst!(down_energy[2], j)
				...
				return (
					left_energy[1] + energies[i,j], 
					# push!(left_energy[2], j)
					pushfirst!(left_energy[2], j-1)
				)
			end
		else
			left_energy = least_energy(energies, i+1, j-1)
			...
			if (left_energy[1] + energies[i,j]) < (right_energy[1] + energies[i,j]) < (down_energy[1] + energies[i,j])
				return (
					left_energy[1] + energies[i,j], 
					# push!(left_energy[2], j)
					pushfirst!(left_energy[2], j-1)
				)
			elseif (right_energy[1] + energies[i,j]) < (down_energy[1] + energies[i,j])
				return (
					right_energy[1] + energies[i,j], 
					# push!(right_energy[2], j)
					pushfirst!(right_energy[2], j+1)
...
function recursive_seam(energies, starting_pixel)
	...
	least_e = least_energy(energies, 1, starting_pixel)
	
	return pushfirst!(least_e[2], starting_pixel)
end
...
function memoized_least_energy(energies, i, j, memory)
	...
	if haskey(memory, (i,j)) return memory end
	
	if i == m-1
		
		if j == 1
			memory[(i,j)] = (energies[i+1,j+1] < energies[i+1,j]
				? memory[(i+1,j+1)] + energies[i,j]
				: memory[(i+1,j)] + energies[i,j])
		elseif j == n
			memory[(i,j)] = (energies[i+1,j] < energies[i+1,j-1]
				? memory[(i+1,j)] + energies[i,j]
				: memory[(i+1,j-1)] + energies[i,j])
		else
			left_energy = energies[i+1, j-1] + energies[i,j]
			down_energy = energies[i+1, j] + energies[i,j]
			right_energy = energies[i+1,j+1] + energies[i,j]
			
			if left_energy < right_energy < down_energy
				memory[(i,j)] = left_energy
			elseif right_energy < down_energy
				memory[(i,j)] = right_energy
			else
				memory[(i,j)] = down_energy
			end
		end
		...
		if j == 1
			memory = memoized_least_energy(energies, i+1, j,   memory)
			memory = memoized_least_energy(energies, i+1, j+1, memory)
			
			right_energy = memory[(i+1,j+1)] + energies[i,j]
			down_energy =  memory[(i+1,j  )] + energies[i,j]
			
			memory[(i,j)] = (right_energy < down_energy
				? right_energy
				: down_energy)
		elseif j == n
			...
			left_energy = memory[(i+1,j-1)] + energies[i,j]
			down_energy = memory[(i+1,j  )] + energies[i,j]
			
			memory[(i,j)] = (left_energy < down_energy
				? left_energy
				: down_energy)
		else
			memory = memoized_least_energy(energies, i+1, j-1, memory)
...
function recursive_memoized_seam(energies, starting_pixel)
	# initialize dictionary with last row completed
	m, n = size(energies)
	memory = Dict{Tuple{Int,Int}, Float64}([((m,j), energies[m,j]) for j in 1:n])

	memory = memoized_least_energy(energies, 1, starting_pixel, memory)
	
	potential_path = [(i,starting_pixel) for i in 1:m]
	first_col = popfirst!(potential_path)[2]
	
	actual_path = reduce(potential_path; init=[first_col]) do a,b
		next_row = b[1]
		last_col = last(a)
		
		# i suppose a macro would be handy, eh?
		if last_col == 1
			down_e =  memory[(next_row, last_col  )]
			right_e = memory[(next_row, last_col+1)]
			
			return push!(a, (down_e < right_e ? last_col : last_col+1))
			# return push!(a, (next_row, (down_e < right_e ? last_col : last_col+1)))
		elseif last_col == n
			left_e =  memory[(next_row, last_col-1)]
			down_e =  memory[(next_row, last_col  )]
			
			return push!(a, (down_e < left_e ? last_col : last_col-1))
			# return push!(a, (next_row, (down_e < left_e ? last_col : last_col-1)))
		else
			left_e =  memory[(next_row, last_col-1)]
			down_e =  memory[(next_row, last_col  )]
			right_e = memory[(next_row, last_col+1)]
			
			next_col = if left_e < down_e < right_e
				last_col-1
			elseif down_e < right_e
				last_col
			else 
				last_col+1
			end
			
			return push!(a, next_col)
			# return push!(a, (next_row, next_col))
		end
	end
...
# ╔═╡ f9d372f0-75b2-11eb-3a95-236a89285223
recursive_memoized_seam(energy(img), 3)
...
function matrix_memoized_least_energy(energies, i, j, memory)
	...
	if memory[i,j] != Base.Inf return memory end
	...
	if i == m-1
		...
		if j == 1
			memory = matrix_memoized_least_energy(energies, i+1, j,   memory)
			memory = matrix_memoized_least_energy(energies, i+1, j+1, memory)
...
function least_energy_matrix(energies)
	...
	le_matrix[:,n] = energies[:,n]
	
	reduce(m-1:-1:1, init=le_matrix) do a,i
		
		this_row_energy = map(1:n) do j
			this_energy = energies[i,j]
						
			min_energy = if j == 1
				down_e =  energies[i+1,j  ]
				right_e = energies[i+1,j+1]
				
				down_e < right_e ? down_e : right_e
			elseif j == n
				left_e =  energies[i+1,j-1]
				down_e =  energies[i+1,j  ]
				
				down_e < left_e ? down_e : left_e
			else
				left_e =  energies[i+1,j-1]
				down_e =  energies[i+1,j  ]
				right_e = energies[i+1,j+1]
				
				if down_e < left_e < right_e
					down_e
				elseif left_e < right_e
					left_e
				else
					right_e
				end
...
function seam_from_precomputed_least_energy(energies, starting_pixel::Int)
	...
	actual_path = reduce(potential_path; init=[first_col]) do a,b
		...
		# i suppose a macro would be handy, eh?
		if last_col == 1
			down_e =  least_energies[next_row, last_col  ]
			right_e = least_energies[next_row, last_col+1]
			
			return push!(a, (down_e < right_e ? last_col : last_col+1))
		elseif last_col == n
			left_e =  least_energies[next_row, last_col-1]
			down_e =  least_energies[next_row, last_col  ]
...
# ╔═╡ 530dba10-733d-11eb-3157-8b17dcb61d8f
Gray.(energy(pika))
