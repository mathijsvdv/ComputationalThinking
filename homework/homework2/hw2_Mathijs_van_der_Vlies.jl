...
# ╔═╡ 11c977b0-5db2-11eb-2a11-670bbb5ac68c
size(img)

# ╔═╡ e49235a4-f367-11ea-3913-f54a4a6b2d6b
no_vcat_observation = md"""
We reduced the number of allocations by 342. This is exactly the number of rows in the image. What happened is that, for each row, we eliminated the allocation of a new array in memory that `vcat` would produce. Instead the existing slices were used directly.
"""
...
Through @view we were able to reduce the number of allocations by 2*342, i.e. twice the number of rows of the image. Effectively we eliminated for each row the allocation of two arrays (the array left and right of the removed pixel) by using views rather than slices.

The end result is quite dramatic: we reduced the minimum evaluation time by half compared to the first example!
"""
...
function greedy_seam(energies, starting_pixel::Int)
	...
	for i in 2:n
		prev_seam = seam[i-1]
		left = max(prev_seam - 1, 1)
		right = min(prev_seam + 1, m)
		
		seam[i] = argmin(@view energies[i, left:right]) + left - 1
	end
...
# ╔═╡ 7eafb770-5db9-11eb-107a-b9b721484747


# ╔═╡ 7ddee6fc-f394-11ea-31fc-5bd665a65bef
greedy_test = Gray.(rand(Float64, (8,10)));

# ╔═╡ 730655a0-5db9-11eb-2ccc-b5ffe1349e9f
...
# ╔═╡ ce2ff170-5dbe-11eb-1126-c5265692f909
# Aside from being inefficient because it enumerates all paths, the `least_energy`
# function also doesn't remember the full path taken, meaning that we have to
# recompute the subpath at i+1 each time until we get to n. This raises the 
# inefficiency by the order of n again.
#
# Better might be to return the path that leads to the lowest possible sum energy, 
# along with the sum itself.
# There is a trade-off though: returning the whole path means growing an array by one 
# element at a time. This allocation of new arrays can get quite inefficient. 
# 

"""Returns the lowest possible sum energy at pixel (i, j), along with the path that gets you there"""
function least_energy_path(energies, i, j)
	n, m = size(energies)
	energy = energies[i, j]
	if i == n
		return energy, [j]
	...
	left = max(j-1, 1)
	right = min(j+1, m)
	min_energies = [least_energy_path(energies, i+1, j) for j in left:right]
	min_energy, arg_min_energy = findmin([e[1] for e in min_energies])
	min_path = min_energies[arg_min_energy][2]
	min_energy += energy
	
	return min_energy, [j; min_path]
end

# ╔═╡ 319885d0-5dc2-11eb-1f42-31ec1482cb9c
@benchmark least_energy(greedy_test, 1, 7)

# ╔═╡ 7e990f42-5dc6-11eb-2979-ff4da38ad4d1
md"We see that `least_energy_path` is three times slower than `least_energy`"
...
function recursive_seam_path(energies, starting_pixel)
	
	return least_energy_path(energies, 1, starting_pixel)[2]
end

# ╔═╡ 1d55333c-f393-11ea-229a-5b1e9cabea6a
md"Compute shrunk image: $(@bind shrink_recursive CheckBox())"

# ╔═╡ a88a0f20-5dc6-11eb-3208-3112a1d3199b
md"Unfortunately, the time gained in eliminating the loop of `seam_recursive` is not enough to offset the time lost in array allocation"
...
- In order to find the path of minimum energy at (i, j), the algorithm considers all neighbors of j at i+1 and run the algorithm on these points recursively. Only at i=n (the number of rows) does the algorithm stop. This effectively searches for all possible paths.
- The number of possible paths grows approximately at rate $\mathcal{O}(3^n)$: at every row i, there is a new option for 3 (sometimes 2, depending on the boundary) neighbors.
"""
...
# ╔═╡ 48c03d00-5e1e-11eb-3bf5-c546cc1c001d
begin
...
# ╔═╡ a4597cd0-5e1e-11eb-2d18-9dfdfc385ef2
get(memory, (3, 2)) do
	return 1 + 2
end
...
begin	
	...
	function memoized_least_energy(energies, i, j)
		memory = Dict{Tuple{Int,Int}, Tuple{Float64,Int}}()
		return memoized_least_energy(energies, i, j, memory)
	end
end
...
# ╔═╡ 2625e9d0-5e22-11eb-168f-c1537eff18e2
A = [1 2; 3 4]
...
begin
	...
	function matrix_memoized_least_energy(energies, i, j)
		memory = fill((0.0, 0), size(energies))
		return memoized_least_energy(energies, i, j, memory)
	end
end
...
# ╔═╡ afa9716e-5dc5-11eb-07e4-dfb17671a79a
if shrink_recursive
	@benchmark shrink_n(pika, 3, recursive_seam)
end

# ╔═╡ 0613a2b0-5dc6-11eb-0e1a-a5241253c521
if shrink_recursive
	@benchmark shrink_n(pika, 3, recursive_seam_path)
end

# ╔═╡ b3d41bd0-5e27-11eb-3210-236acc4a28d9
@benchmark shrink_n(pika, 3, recursive_memoized_seam)

# ╔═╡ 44fb0890-5e27-11eb-12c0-13f0aa3eb6dd
@benchmark shrink_n(pika, 3, matrix_memoized_seam)
