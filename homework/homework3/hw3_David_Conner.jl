...
unused_letters = let
	freq = letter_frequencies(clean(first(samples)))
	reduce([i for i in 1:27]; init=[]) do a,i
		if freq[i] == 0 push!(a, alphabet[i]) end
		return a
	end
end
...
double_letters = map(filter(i -> sample_freq_matrix[i,i] > 0, 1:27)) do c
	alphabet[c]
end
...
most_likely_to_follow_w = begin 
	freq_w_post = sample_freq_matrix[23,:]
	w_post_idx = reduce(1:27, init=1) do a,i
		if freq_w_post[i] > freq_w_post[a] return i else return a end
	end
	alphabet[w_post_idx]
end
...
# ╔═╡ adecf250-75d8-11eb-2c13-0b93f1af9115
most_likely_to_precede_w = begin 
	freq_w_pre = sample_freq_matrix[:,23]
	w_pre_idx = reduce(1:27, init=1) do a,i
		if freq_w_pre[i] > freq_w_pre[a] return i else return a end
	end
	alphabet[w_pre_idx]
end
...
row_col_answer = md"""
these are the frequencies of a character preceding or following any other letter. this is a tensor contraction of sorts on the conditions set for `p(char | preceeding)` or `p(char | following)`. these become `p(char | preceded_by_any)` and `p(char | followed_by_any)`. 

if underscore in the table is interpreted as ' ', then these should normalize to being equal, since there is a kind of "gauge" symmetry for preceding/following. i.e. all characters are in the same string and index `n` is always preceeded by index `n-1` and followed by index `n+1` except at the beginning and end of the string, respectively. if newlines break the text into multiple strings, then these two probability distributions become more dissonant (i.e. the results of the contraction on conditions `p(char | preceded_by_any)` and `p(char | followed_by_any)` develop a greater difference).
"""
...
# ╔═╡ 39eee8d8-75dd-11eb-26b4-ff15460ee990
function l1_norm(A, B)
	sum(abs.(A-B))
end

# ╔═╡ 13c89272-f934-11ea-07fe-91b5d56dedf8
function matrix_distance(A, B)
	l1_norm(A,B) # do something with A .- B
end
...
function word_counts(words::Vector)
	...
	return reduce(words; init=counts) do counts,word
		if haskey(counts, word) counts[word] += 1 else counts[word] = 1 end
		return counts
	end
end
...
function completions_cache(grams)
	...
	return reduce(grams; init=cache) do n1_grams, n_gram
		n1_gram, gram = n_gram[1:end-1], last(n_gram)
		if haskey(n1_grams, n1_gram) 
			push!(n1_grams[n1_gram], gram) 
		else 
			n1_grams[n1_gram] = [gram]
		end
		return n1_grams
	end
end
